...
export default function TokenManagementPage() {
  ...
  const updateGenerateForm = (key: keyof GenerateFormState, value: string) => {
  ...
  const generateToken = async () => {
    ...
    try {
      ...
      const payload: any = {
        album_id: Number(generateForm.album_id),
      ...
      setGenerateForm({ album_id: "", client_id: "", client_name: "", email: "" });
      await loadSessions();
  ...
  const addAlbumToToken = async (tokenValue: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API}/admin/token-management/${tokenValue}/albums`, {
        ...
        body: JSON.stringify({ album_id: Number(albumId) }),
      });
      ...
      <section className="bg-white/5 border border-white/10 rounded-2xl p-5 shadow-lg shadow-black/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mt-4">
          <LabeledInput
            label="Album ID"
            placeholder="123"
            value={generateForm.album_id}
            onChange={(v) => updateGenerateForm("album_id", v)}
          />
          <LabeledInput
            label="Existing client ID (optional)"
            placeholder="45"
            value={generateForm.client_id}
            onChange={(v) => updateGenerateForm("client_id", v)}
          />
          <LabeledInput
        ...
        <p className="text-xs text-neutral-400 mt-2">
          Provide a client ID to add a gallery to an existing client. If only email is provided, a new client record is created or reused.
        </p>
      </section>
              ...
              <div className="mt-6 space-y-3">
                ...
                <div className="grid gap-3">
                  {clientSessions.map((session) => (
                    <div
                      key={session.id}
                      className="border border-white/10 rounded-xl p-3 flex flex-wrap items-center justify-between gap-3"
                    >
                      <div className="space-y-1">
                        ...
                        <p className="text-neutral-400 text-xs">Album #{session.album_id}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <a
